...
open class InAppUpdateManager(val model: InAppUpdateModel) {
    ...
    fun checkUpdate(info: AppUpdateInfo) {
        if (!model.isAvailableToShowUpdateUi()) {
            log("not ready to show update UI")
            return
        }
    ...
    interface InAppUpdateModel {
        ...
        fun isAvailableToShowUpdateUi(): Boolean
    }
