...
class DownloadInfoManager {
    ...
    fun delete(rowId: Long, listener: AsyncDeleteListener?) {
        mQueryHandler!!.startDelete(TOKEN, AsyncDeleteWrapper(rowId, listener)
            , DownloadContract.Download.CONTENT_URI, DownloadContract.Download._ID + " = ?", arrayOf(java.lang.Long.toString(rowId)))
    }

    fun updateByDownloadId(downloadInfo: DownloadInfo, listener: AsyncUpdateListener?) {
        mQueryHandler!!.startUpdate(TOKEN, listener, DownloadContract.Download.CONTENT_URI, getContentValuesFromDownloadInfo(downloadInfo)
            , DownloadContract.Download.DOWNLOAD_ID + " = ?", arrayOf(java.lang.Long.toString(downloadInfo.downloadId!!)))
    }

    fun updateByRowId(downloadInfo: DownloadInfo, listener: AsyncUpdateListener?) {
        mQueryHandler!!.startUpdate(TOKEN, listener, DownloadContract.Download.CONTENT_URI, getContentValuesFromDownloadInfo(downloadInfo)
            , DownloadContract.Download._ID + " = ?", arrayOf(java.lang.Long.toString(downloadInfo.rowId!!)))
    }
    ...
    fun queryByDownloadId(downloadId: Long?, listener: AsyncQueryListener?) {
        ...
        mQueryHandler!!.startQuery(TOKEN, listener, Uri.parse(uri), null, DownloadContract.Download.DOWNLOAD_ID + "==?", arrayOf(java.lang.Long.toString(downloadId!!)), null)
    }

    fun queryByRowId(rowId: Long?, listener: AsyncQueryListener?) {
        val uri = DownloadContract.Download.CONTENT_URI.toString()
        mQueryHandler!!.startQuery(TOKEN, listener, Uri.parse(uri), null, DownloadContract.Download._ID + "==?", arrayOf(java.lang.Long.toString(rowId!!)), null)
    }
        ...
        queryByDownloadId(downloadId, object : AsyncQueryListener {
            override fun onQueryComplete(downloadInfoList: List<*>) {
                for (i in downloadInfoList.indices) {
                    val queryDownloadInfo = downloadInfoList[i] as DownloadInfo
                    ...
                    if (downloadId == queryDownloadInfo.downloadId) {
                        val rowId = queryDownloadInfo.rowId!!
                        val newInfo = pojoToDownloadInfo(pojo, newPath, rowId)
    ...
    fun showOpenDownloadSnackBar(rowId: Long?, container: View, logTag: String?) {
        queryByRowId(rowId, AsyncQueryListener { downloadInfoList: List<DownloadInfo> ->
            val existInLocalDB = downloadInfoList.size > 0
            if (!existInLocalDB) {
            ...
            if (existInDownloadManager) {
                snackbar.setAction(R.string.open) { view: View? ->
                    try {
    ...
    companion object {
        const val DOWNLOAD_OPEN = "download open"
        ...
        private var mQueryHandler: DownloadInfoQueryHandler? = null
