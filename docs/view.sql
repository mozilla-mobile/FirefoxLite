...
SELECT
    ...
    CASE
        ...
        WHEN (event_category IN ('action') ) AND (event_method IN ('swipe') ) AND (event_object IN ('logoman') ) THEN 'Rocket -  Swipe Logoman' 
        ...
        WHEN (event_category IN ('action') ) AND (event_method IN ('open') ) AND (event_object IN ('firstrun_push') ) THEN 'Rocket -  Open D1 Notification' 
